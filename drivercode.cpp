...
{
    string path;
    cout << "Write path of your .txt file which contains expressions in order : ";
    cin >> path;
    ...
    std::string evalFilePath = "eval_output.txt";
    std::ofstream ef(evalFilePath.c_str());
    std::ofstream evalFile(evalFilePath, ios::app);
    if (!evalFile.is_open())
    {
    ...
    vector<string> output = evaluator.print_result();
